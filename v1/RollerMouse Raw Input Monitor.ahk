...
Gui, Add, Edit, xm ym w1100 h650 ReadOnly -Wrap HwndgEdit
Gui, Show,, RollerMouse Raw Input Monitor
...
EnumerateAndRegister()
{
    ...
    count := 0
    result := DllCall("GetRawInputDeviceList"
        , "Ptr", 0
        , "UInt*", count
        , "UInt", entrySize
        , "UInt")
    ...
    VarSetCapacity(list, count * entrySize, 0)
...
GetDeviceMeta(hDevice, type)
{
    ...
    if (chars > 0)
    {
        VarSetCapacity(nameBuf, (chars + 1) * 2, 0)
        chars2 := chars
    ...
    if RegExMatch(path, "i)VID_([0-9A-F]{4}).*PID_([0-9A-F]{4})", m)
    {
        meta["vid"] := m1
        meta["pid"] := m2
    }
...
HexDump(ByRef buf, offset, length)
{
    result := ""
...
Log(text)
{
    ...
    VarSetCapacity(selectionStart, A_PtrSize, 0)
    VarSetCapacity(selectionEnd, A_PtrSize, 0)
    ...
    ; Edit末尾へキャレットを移動して追記
    textLength := DllCall("SendMessage"
        , "Ptr", gEdit
